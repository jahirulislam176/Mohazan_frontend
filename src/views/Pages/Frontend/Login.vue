<template>
    <!-- Page Content -->
    <div class="content">
        <div class="container-fluid">

            <div class="row">
                <div class="col-md-8 offset-md-2">

                    <!-- Login Tab Content -->
                    <div class="account-content">
                        <div class="row align-items-center justify-content-center">
                            <div class="col-md-7 col-lg-6 login-left">
                                <img src="assets/img/login-banner.png" class="img-fluid" alt="Doccure Login">
                            </div>
                            <div class="col-md-12 col-lg-6 login-right">
                                <div class="login-header">
                                    <h3>Login <span>Customer</span></h3>
                                </div>
                                <form @submit.prevent="login">

                                    <div class="form-group form-focus">
                                        <input type="email" class="form-control floating" v-model="credentials.email">
                                        <label class="focus-label">Email</label>
                                    </div>
                                    <div class="form-group form-focus">
                                        <input type="password" class="form-control floating"
                                            v-model="credentials.password">
                                        <label class="focus-label">Password</label>
                                    </div>
                                    <!-- <InputF class="col-md-12" v-for="field in fields" :key="field.id"
                                        :label="field.label" :placeholder="field.placeholder" :type="field.type"
                                        :class="field.class">
                                    </InputF> -->

                                    <button class="btn btn-primary btn-block btn-lg login-btn"
                                        type="submit">Login</button>
                                    <div class="login-or">
                                        <span class="or-line"></span>
                                        <span class="span-or">or</span>
                                    </div>
                                    <div class="row form-row social-login">
                                        <div class="col-6">
                                            <a href="#" class="btn btn-facebook btn-block"><i
                                                    class="fab fa-facebook-f mr-1"></i> Login</a>
                                        </div>
                                        <div class="col-6">
                                            <a href="#" class="btn btn-google btn-block"><i
                                                    class="fab fa-google mr-1"></i> Login</a>
                                        </div>
                                    </div>
                                    <div class="text-center dont-have">Don’t have an account?
                                        <router-link :to="{name:'Register'}">Register</router-link>
                                    </div>
                                </form>
                            </div>
                        </div>
                    </div>
                    <!-- /Login Tab Content -->

                </div>
            </div>

        </div>

    </div>
    <!-- /Page Content -->

</template>

<script>
import InputF from '../../components/Field/Input.vue';
export default{
    name:"Login",
    components:{
        InputF
    },
    data(){
        return{
            credentials:{
                email:"makamamunia@gmail.com",
                password:12345678,
            }
           
        }
    },
    methods:{
        login(context,credentials){
            // this.$store.dispatch('login', this.credentials);
            this.$store.dispatch('login', this.credentials).then(res => {
                // toastr.success(res.message);
                console.log(res);
                    this.$router.push({
                        name: "Dashboard",
                    });
            }).catch((error) => {
                    console.log(error)
                //     const data = error.response.data;
                //     if (typeof data === "object" && typeof data.errors === "object") {
                //         for (const [k, v] of Object.entries(data.errors)) {
                //             // toastr.error(v);
                //         }
                //     } else {
                //         // toastr.error(data.message);
                //     }
                }); 
           
        }
    }
}
// fields:[
            //   {
            //         label: "Enter your email",
            //         placeholder:"Enter your email",
            //         type:"text"
            //   },
            //   {
            //         label: "Enter your password",
            //         placeholder:"Enter your password",
            //         type:"password"
            //   }
            // ]
</script>

 